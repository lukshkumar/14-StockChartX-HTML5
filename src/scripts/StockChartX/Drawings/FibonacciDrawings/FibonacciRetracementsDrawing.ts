...
export interface IFibonacciRetracementsDrawingDefaults
  extends IFibonacciDrawingBaseDefaults {
  reverse: boolean;
  levelLinesExtension: string;
}

let defaults: IFibonacciRetracementsDrawingDefaults = {
  levels: [
    { value: 0 },
    { value: 0.2361 },
    { value: 0.382 },
 ...
 * @constructor StockChartX.FibonacciRetracementsDrawing
 * @augments StockChartX.FibonacciDrawingBase
 * @example
 *  // Create fibonacci retracements drawing.
 *  var retracements1 = new StockChartX.FibonacciRetracementsDrawing({
 *      points: [{x: 10, y: 20}, {x: 50, y: 60}]
 *  });
 *
 *  // Create fibonacci retracements drawing.
 *  var retracements2 = new StockChartX.FibonacciRetracementsDrawing({
 *      points: [{record: 10, value: 20.0}, {record: 50, value: 60.0}]
 *  });
 *
 *  // Create fibonacci retracements drawing with a custom theme.
 *  var retracemnts3 = new StockChartX.FibonacciRetracementsDrawing({
 *      points: [{record: 10, value: 20.0}, {record: 50, value: 60.0}],
 *      theme: {
 *          line: {
 *              strokeColor: '#FF0000'
 ...
 *  });
 */
export class FibonacciRetracementsDrawing extends FibonacciDrawingBase {
  static get className(): string {
    return "fibonacciRetracements";
  }

  static defaults: IFibonacciRetracementsDrawingDefaults = defaults;
  ...
  get reverse(): boolean {
    ...
    return flag != null
      ? <boolean>flag
      : FibonacciRetracementsDrawing.defaults.reverse;
  }
  ...
  get levelLinesExtension(): string {
    ...
    return extension != null
      ? <string>extension
      : FibonacciRetracementsDrawing.defaults.levelLinesExtension;
  }
  ...
  bounds(): IRect {
    ...
    y = this.reverse
      ? Math.round((points[0].y - points[1].y) * maxLevel + points[1].y)
      : Math.round((points[1].y - points[0].y) * maxLevel + points[0].y);

    let point1 = { x: xRange.min, y },
      point2 = { x: xRange.max, y };

    return {
      left: Math.min(points[0].x, points[1].x, point1.x, point2.x),
      top: Math.min(points[0].y, points[1].y, point1.y, point2.y),
      width: Math.abs(
        Math.max(points[0].x, points[1].x) - Math.min(points[0].x, points[1].x)
      ),
      height: Math.abs(
        Math.max(points[0].y, points[1].y, point1.y, point2.y) -
          Math.min(points[0].y, points[1].y, point1.y, point2.y)
      )
    };
  }
  ...
  hitTest(point: IPoint): boolean {
    ...
    if (this.showLevelLines || this.showLevelBackgrounds) {
      ...
      for (let level of this.levels) {
        ...
        let y = this.reverse
            ? Math.round(
                (points[0].y - points[1].y) * level.value + points[1].y
              )
            : Math.round(
                (points[1].y - points[0].y) * level.value + points[0].y
              ),
