...
import { LinePriceStyle, LinePlot } from "../../../src/scripts/exporter.ts";
...
describe("LinePriceStyle", () => {
  let target: LinePriceStyle;

  beforeEach(() => {
    target = new LinePriceStyle();
  });

  describe("className", () => {
    it("should return 'line'", () => {
      expect(LinePriceStyle.className).to.equal("line");
    });
  });
  ...
  describe("createPlot", () => {
    ...
    it("should be OHLC bar plot", () => {
      expect(target.createPlot().plotStyle).to.equal(LinePlot.Style.SIMPLE);
    });
